...
export default function ExaminerOsceDashboard() {
  const t = useTranslations("examExaminerOsce");
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "ONGOING":
        return <Badge className="bg-blue-500"><Clock className="w-3 h-3 mr-1" /> {t("statusOngoing")}</Badge>;
      case "COMPLETED":
        return <Badge className="bg-green-500"><CheckCircle2 className="w-3 h-3 mr-1" /> {t("statusCompleted")}</Badge>;
      case "DRAFT":
        return <Badge variant="secondary"><AlertCircle className="w-3 h-3 mr-1" /> {t("statusDraft")}</Badge>;
      default:
          ...
          exams.map((exam) => (
            <Card key={exam.id} className="flex flex-col hover:shadow-md transition-shadow">
              <CardHeader className="pb-3">
                ...
                <CardDescription className="line-clamp-2">{exam.description || t("defaultDescription")}</CardDescription>
              </CardHeader>
              <CardContent className="mt-auto pt-4 flex flex-col gap-4">
                <div className="flex justify-between text-sm border-t pt-4">
                  <span className="text-muted-foreground">{t("staseLabel")}</span>
