...
export default function IncidentDetailPage() {
  ...
  // Severity dikonfigurasi via system_references → fallback ke nilai mentah bila tak ada key.
  ...
  const getStatusBadge = (status: IncidentStatus) => {
    switch (status) {
      ...
      case "investigating": return <Badge className="bg-amber-500 text-white">{t("status.investigating")}</Badge>;
      case "resolved": return <Badge className="bg-green-600 text-white">{t("status.resolved")}</Badge>;
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      ...
      toast.error(e.response?.data?.message ?? e.response?.data?.error ?? t("statusUpdateError"));
    } finally {
  ...
  if (!incident) {
    return (
      <div className="text-center py-16 text-muted-foreground">
        {t("notFound")}
        <Button variant="link" onClick={() => router.back()}>{tc("back")}</Button>
      </div>
    );
  }
    ...
    <div className="max-w-4xl mx-auto space-y-6 pb-12">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1 font-mono break-all">{t("idLabel", { id: incident.id })}</p>
          </div>
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-sm">
          <div>
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1">{t("fieldType")}</p>
            <p className="font-medium capitalize">{incident.incident_type.replace(/_/g, " ")}</p>
          </div>
          <div>
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1">{t("fieldDate")}</p>
            <p className="font-medium">{format(new Date(incident.incident_date), "dd MMMM yyyy")}</p>
          </div>
          <div>
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1">{t("fieldLocation")}</p>
            <p>{incident.location}</p>
          </div>
          <div>
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1">{t("fieldReporter")}</p>
          ...
          {incident.involved_parties && (
            <div className="sm:col-span-2">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1">{t("fieldInvolvedParties")}</p>
              <p>{incident.involved_parties}</p>
            </div>
          )}
          <div>
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1">{t("fieldReportedAt")}</p>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">{t("chronologyTitle")}</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      {incident.attachment_path && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base">{t("attachmentTitle")}</CardTitle>
          </CardHeader>
          <CardContent>
            <Button
              variant="outline"
        ...
        <Card className="border-green-200 dark:border-green-900">
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2 text-green-700 dark:text-green-400">
              <CheckCircle2 className="h-4 w-4" /> {t("resolutionTitle")}
            </CardTitle>
          </CardHeader>
          <CardContent>
      ...
      {canManage && incident.status !== "resolved" && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base">{t("updateStatusTitle")}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
              ...
              <Select value={newStatus} onValueChange={(v) => setNewStatus(v ?? "")}>
                ...
                <SelectContent>
                  {incident.status === "submitted" && (
                    <SelectItem value="investigating">{t("startInvestigation")}</SelectItem>
                  )}
            ...
            {newStatus === "resolved" && (
              <div className="space-y-2">
                <Label>{t("resolutionNotesLabel")} <span className="text-red-500">*</span></Label>
                <Textarea
            ...
            >
              {updatingStatus ? tc("saving") : t("saveStatusButton")}
            </Button>
          </CardContent>
        </Card>
      )}
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2">
              ...
              {t("investigationNotesTitle")}
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            {/* Add note form */}
            <div className="space-y-3 p-4 border rounded-lg bg-muted/20">
              <Label className="text-sm font-medium">{t("addNoteLabel")}</Label>
              <Textarea
              ...
              <div className="flex items-center justify-between">
                <label className="flex items-center gap-2 text-sm cursor-pointer">
                  ...
                  <span className="text-muted-foreground">{t("internalNoteLabel")}</span>
                </label>
                <Button
                  size="sm"
                ...
                >
                  {addingNote ? tc("saving") : t("addButton")}
                </Button>
              </div>
            </div>
                  ...
                  <div key={note.id} className="border rounded-lg p-4 space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2 text-sm">
                        <span className="font-medium">{note.author?.name ?? t("adminFallback")}</span>
                        {note.is_internal && (
                          <span className="inline-flex items-center gap-1 text-xs text-amber-700 bg-amber-100 dark:bg-amber-900/30 dark:text-amber-300 px-1.5 py-0.5 rounded">
                            <Lock className="h-3 w-3" /> {t("internalTag")}
                          </span>
                        )}
                      </div>
